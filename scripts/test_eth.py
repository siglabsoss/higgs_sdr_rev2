...
class TestEth:
    ...
    def run_test(self):
        ...
        for idx in range(10):
            ...
            timeout_pkt_cnt = 0
            while(data != expected[idx]):
                (s, data) = self.get_message()
                logger.info('Eth retry, got incorrect value: ' + hex(data))
                timeout_pkt_cnt += 1
                if(timeout_pkt_cnt == 20):
                    break
            if(data != expected[idx]):
                logger.info('Incorrect packet index ' + str(idx))
                logger.info('Expected:' + hex(expected[idx]) + " got " + hex(data))
                did_pass = 0
