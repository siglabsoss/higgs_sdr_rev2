...
static unsigned tdma_subcarrier = 17;
static unsigned int lifetime_32 = 0; // for tdma
...
void callback_eq_setting(unsigned int data) {
    if( data == 0 ) {
        ...
        vmem_all_pilots[tdma_subcarrier] = 0x00002000;
    }
}
...
void __attribute__((optimize("Os"))) cs20_dump_status(unsigned int data) {
    ...
    fb_queue_ring_eth( CS20_REPORT_STATUS_PCCMD | inc | (ud_frame & 0x0000ffff)  ); inc += 0x00010000;
