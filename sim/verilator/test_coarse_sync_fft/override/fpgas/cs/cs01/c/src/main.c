...
void trig_dma_in(unsigned int idx, unsigned int timer_start) {
  ...
  unsigned int start_addr = VMEM_DMA_ADDRESS(dma_in_ptr[idx]);

  CSR_WRITE(DMA_0_START_ADDR, start_addr);
...
void setup_dma_in(void) {
  ...
  make_nco(VMEM_DMA_ADDRESS(nco_in_ptr[0]), 1024, nco_angle, nco_delta);
  nco_angle = (nco_angle + nco_angle_delta);
  make_nco(VMEM_DMA_ADDRESS(nco_in_ptr[1]), 1024, nco_angle, nco_delta);
  nco_angle = (nco_angle + nco_angle_delta);
...
void pet_dma_in(void) {
  ...
  if(helper & DMA_0_ENABLE_BIT) {
    ...
    SET_REG(x3, 0xa0000000 | dma_state);
  ...
  if(fft_ready != -1) {
    ...
    make_nco(VMEM_DMA_ADDRESS(nco_in_ptr[fft_ready]), 1024, nco_angle, nco_delta);
    nco_angle = (nco_angle + nco_angle_delta);
...
void pet_fft() {
  ...
  if( dma_in_valid != -1 )
  {
    SET_REG(x3, 0xb0000000 | dma_in_valid);
    ///////
    //
...
void xbb_coarse_sync(CFO_Results *CFO_results, unsigned int coarse_sync_number) 
{
  ...
  for(int index=0; index<coarse_sync_number; index++)
  {
    ...
    // CSR_READ(TIMER_VALUE, start_clk);
    xbb_conj_multi(VMEM_ADDRESS(config_word_conj_eq_0f), input_conj_multi_location_0, input_conj_multi_location_1, output_conj_multi_location);
    // CSR_READ(TIMER_VALUE, end_clk);   
    exp_data_location=exp_data_1280_ext_location;

    xbb_onetone_calculation(VMEM_ADDRESS(config_word_cmul_rx4_0f), output_conj_multi_location, exp_data_location, output_onetone_calculation_location, bank_address_onetone_calculation_location, permutation_address_onetone_calculation_location, all_one_location);
  ...
  //set fake cfo for test purpose
  ring_block_send_eth(CFO_results->cfo_estimate);
  CFO_results->cfo_estimate = 100;
  ring_block_send_eth(CFO_results->cfo_estimate);

  

  if(CFO_results->cfo_estimate>0x7fffffff)
  {
  	nco_delta = (0x100000000-CFO_results->cfo_estimate) * 131.072 + 0.5;
  	cfo_compensation_direction = 0;
  	nco_angle_delta = (0x100000000-CFO_results->cfo_estimate)*167772.16+0.5;
  }
  	
  else
  {
  	nco_delta = CFO_results->cfo_estimate *131.072+0.5;
  	cfo_compensation_direction = 1;
  	nco_angle_delta = CFO_results->cfo_estimate*167772.16+0.5;
  }
  	

  // symbol timing shift, throw away these samples
  //set fake symbole timing offset
  ring_block_send_eth(sample_cp_advance);
  sample_cp_advance=8*16;
  ring_block_send_eth(sample_cp_advance);
...
int main(void) {
 ...
 ///////////////////////////
    ...
    ring_block_send_eth((nco_angle_delta>>16));
    ring_block_send_eth((nco_angle_delta));
    ring_block_send_eth(nco_delta);
    ring_block_send_eth(cfo_compensation_direction);
