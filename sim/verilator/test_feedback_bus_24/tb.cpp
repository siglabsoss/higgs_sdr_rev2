...
int main(int argc, char** argv, char** env) {
  ...
  std::vector<uint32_t> a_hash;
  std::vector<uint32_t> b_hash;

  cout << "\n";

  t->registerRawFbCb([&](const feedback_frame_t *v) {
    uint32_t hash = _feedback_hash(v);
    // cout << "A hash: " << HEX32_STRING(hash) << "\n";
    a_hash.push_back(hash);
  });
  ...
  t->registerFbCb([&](uint32_t t0, uint32_t t1, const std::vector<uint32_t>& header, const std::vector<uint32_t>& body) {
    ...
    if( t0 == FEEDBACK_TYPE_STREAM && body.size() > 1 ) {
        auto prev = body[0];
        for(uint32_t i = 1; i < body.size(); i++) {
            auto w = body[i];
            if( w != (prev+1)) {
                cout << "MISMATCH PREV " << HEX32_STRING(w) << " " << HEX32_STRING(prev) << "\n";
            }
  ...
  // t->enableParseFeedbackBus(false);
  ...
  for(uint32_t i = 0; i < t->outs["cs20out"]->data.size(); /*empty*/) {
      ...
      if(word != 0) {
          ...
          uint32_t hash = _feedback_hash((feedback_frame_t*)v);
          // cout << "B hash: " << HEX32_STRING(hash) << "\n";
          b_hash.push_back(hash);
  ...
  unsigned min_hash = std::min(a_hash.size(), b_hash.size());

  assert(a_hash.size() > 15 && "a_hash is too small");
  assert(b_hash.size() > 15 && "b_hash is too small");

  unsigned mismatch = 0;

  for(unsigned i = 0; i < min_hash; i++) {
    const auto a = a_hash[i];
    const auto b = b_hash[i];
    cout << HEX32_STRING(a) << " " << HEX32_STRING(b);
    if(a != b) {
        cout << "  MISMATCH!!!";
        mismatch++;
    }
