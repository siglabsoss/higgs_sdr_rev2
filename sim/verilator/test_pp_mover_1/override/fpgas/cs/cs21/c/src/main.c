...
#define PING_PONG_BUFFER_SIZE_IN (1024+TRUNK_LENGTH)
...
void __attribute__((always_inline)) handle_grc_stream(const unsigned* const cpu_in) {
    ...
    if( zero_output_mode ) {
        // does the current progress fall within the tagged region?
        bool tag = duplex_tag_section(duplex_progress, zero_output_mode);

        zero_stream = !tag;  // if we are not in the region, zero
    }
...
void __attribute__((always_inline)) handle_fine_sync(const unsigned* cpu_in) {
    ...
    dma_block_send_finalized(  VMEM_DMA_ADDRESS(cpu_in) + FFT_SIZE, FINE_SYNC_LENGTH, 1);
...
unsigned do_work(
    ...
    const unsigned int input_row = VMEM_ROW_ADDRESS(cpu_in);
    (void)input_row;

    lifetime_32 = cpu_in[FFT_SIZE + TRUNK_FRAME_COUNTER];

    duplex_progress = lifetime_32 % DUPLEX_FRAMES;

    duplex_mode = get_duplex_mode(&duplex, duplex_progress);
    ...
    if( unlikely(forward_eq) ) {
        ...
        if( duplex_mode_rx != DUPLEX_SEND_UL_FEEDBACK ) {
            // we are the receiver. the transmitter is sending us pilot tones
            vec_eq_feedback_analog.seq = lifetime_32;
            send_feedback_bus_header(&vec_eq_feedback_analog);
            dma_block_send_finalized( VMEM_DMA_ADDRESS(input_dma), FFT_SIZE, 1);
        } else {
    ...
    */
    ...
    /* FIXME uncomment and fix
    if( unlikely(frame_num_all_output_counter == frame_num_all_output_period) ) {
        // send header
    ...
    */
    ...
    // const unsigned int mover_input_increment_row = (dma_idx_just_finished * (FFT_SIZE+DMA_IN_EXTRA)) / NSLICES;


    handle_grc_stream(cpu_in);

    handle_fine_sync(cpu_in); // FIXME
