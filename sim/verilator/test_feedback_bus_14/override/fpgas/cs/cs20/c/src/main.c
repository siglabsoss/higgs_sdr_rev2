...
uint32_t mapmov_frame_offset = 0;
uint32_t frame_phase_p = 0;
void pet_eq_and_dma_out(void) {
            ...
            if( ud_epoc != 0 && (ud_epoc < epoc) ) {
                ...
                if( (ud_timeslot < timeslot) ) {
                    ...
                    if( can_tx ) {
                        ...
                        mapmov_frame_offset = frame_phase_p + 1;
                        if( mapmov_frame_offset >= 5 ) {
                            mapmov_frame_offset -= 5;
                        }
                    } else {
    ...
    if(can_tx) {
        if(
            ...
            // output_dma_offset = (mapmov_frame_offset*1024); // new
            output_dma_offset = (frame_phase*1024); // old
            input_dma_offset = 0;
