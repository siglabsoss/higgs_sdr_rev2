...
VMEM_SECTION unsigned short variable_coarse_mul[32];
VMEM_SECTION unsigned short variable_coarse_fft[32];
VMEM_SECTION unsigned short variable_coarse_add[32];


VMEM_SECTION unsigned int cfo_nco_data[1280] = {0};

// we write to these using copy_config
VMEM_SECTION unsigned short cfg_cfo_mul[32];
VMEM_SECTION unsigned short cfg_cfo_mul_conj[32];


unsigned int variable_coarse_mul_row;
unsigned int variable_coarse_fft_row;
unsigned int variable_coarse_add_row;
...
#define DMA_OUT_TRUNK (TRUNK_LENGTH)
...
static const unsigned int nco_length = 1024+256;
#define NCO_BUFFER_SIZE  (1280)
#include "nco_driver.h"
...
void __attribute__((optimize("Os"))) sfo_sign_callback(unsigned int data) {
    ...
    if((data == 1 || data == 2) && ringbus_sfo_adjustment_temp == 0)
    {
    	data = 0;
    }
...
VMEM_SECTION unsigned int fft_buffer_a[1024+256];
VMEM_SECTION unsigned int fft_buffer_b[1024+256];
...
void setup_fft(void) {
  // fft_ptr[0] = (unsigned int)(unsigned int*) vmalloc_single(&mgr);
  // fft_ptr[1] = (unsigned int)(unsigned int*) vmalloc_single(&mgr);
  fft_ptr[0] = (unsigned int) fft_buffer_a;
  fft_ptr[1] = (unsigned int) fft_buffer_b;
  mark_both_fft_empty();
...
void pet_fft() {
  ...
  if( dma_in_valid != -1 )
  {
    ...
    ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    //////////////////////// for cfo correction   this is a ping-pong method////////////////////////////////////////////////////////////////////////
    ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    if(sync_finished_hold == 1)
    {
    	...
    	unsigned int flag = nco_schedule_next_cfo(nco_length, current_angle, rbus_omega);
    	if(flag == 1)
    	{
    		next_angle = current_angle + nco_length*rbus_omega;
    	}
        ...
        int nco_ready_indi = nco_ready(&cpu_pointer);


        if((nco_ready_indi == 1) ||(nco_ready_indi == 0))
        {
    	    run_cfo(rbus_omega_sign, VMEM_ROW_ADDRESS(cpu_ptr_from_dma), VMEM_ROW_ADDRESS(cpu_pointer), VMEM_ROW_ADDRESS(cpu_ptr_from_dma));
    	    nco_done(nco_ready_indi);
        }
    ...
    //     {
	   ...
	   //  	flag = nco_schedule_next_cfo(nco_length, current_angle, rbus_omega);
	   //  	if(flag == 1)
	   //  	{
	   //  		next_angle = current_angle + nco_length*rbus_omega;
	   //  	}
    ...
    //     int nco_ready_indi = -1;

    //     while(nco_ready_indi == -1)
    //     {
...
void pet_dma_out(void) {
  ...
  if( (!output_blocked) && (fft_valid != -1) ) {
    ...
    if( enable_output_stream == 1 ) {
        ...
        frame_track_counter++;
        vector_memory[VMEM_DMA_ADDRESS(cpu_ptr_from_fft) +
                      DMA_OUT_CHUNK +
                      TRUNK_FRAME_COUNTER] = frame_track_counter;
        dma_out_set_safe(VMEM_DMA_ADDRESS(cpu_ptr_from_fft),
                         DMA_OUT_CHUNK + DMA_OUT_TRUNK);
...
void pet_symbol_sync() {
    if( fsm_state == FSM_DO_SYNC ) {
        ...
        sync_finished = 1;
        frame_track_counter += (global_coarse_sync_num*4/5);
    }
...
void app_barrel_shift_callback(const unsigned int data) {
    ...
    unsigned short int* cpu_source;
    unsigned short int* cpu_dest;
...
void upper_callback(unsigned int data) {
    ...
    ring_block_send_eth(0x82000000|cmd);
    ring_block_send_eth(0x83000000|packet_SFO_adjustment_step);
}

void cfo_turnoff_callback()
{
