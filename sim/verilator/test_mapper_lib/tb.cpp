...
std::vector<uint32_t> counter_stream_bump(unsigned int pull) {
  std::vector<uint32_t> out;
  static unsigned int start = 0x192d3afa;
...
int main(int argc, char** argv, char** env) {
  ...
  for(unsigned int i = 0; i < us; i++) {
    ...
    if( i > us_base && ((us_base + i) % adv) == 0) {
      t->inStreamAppend("cs11in", swizzle_reverse_all(counter_stream_bump(64)));
    }

    t->tick(500);
  }

  t->outStreamDump("cs11out");
