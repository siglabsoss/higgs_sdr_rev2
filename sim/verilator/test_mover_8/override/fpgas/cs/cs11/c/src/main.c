...
void xbb_residue_phase(unsigned int input_data_location, unsigned int output_data_location)
{
   ...
   // ATAN(temp_common_angle,pilot_common_phase,15);
   ...
   // unsigned int nco_angle_common_phase = ((0x10000-temp_common_angle)<<16); // with common phase correction at rx
   ...
   //xbb_conj_multi_const(VMEM_ADDRESS(config_word_cmul_eq_0f), input_data_location, VMEM_ADDRESS(nco_data_common_phase), output_data_location);
...
void pet_fft() {
  ...
  if( dma_in_valid != -1 )
  {
    ...
    if(mag_adjust_flag == 0) {
        ...
        for(int index=0; index<65; index++)   //including 16 padding
        {
            ADD_LK13(V0, V0, V2, 0);
            ADD_SK13(V1, V1, V2, 0);
        }
        ...
        // FIXME: this equation is weird, and may involve extra shifts
        channel_data = vector_memory[VMEM_ADDRESS(cpu_ptr_from_dma)*16+mag_counter];
        ...
        if( mag_adjust_flag == 2 ) {
            ...
            channel_data2 = vector_memory[VMEM_ADDRESS(cpu_ptr_from_dma)*16+mag_counter2];
        }
    ...
    //////////////////////////////////////////////////////////////////////
    xbb_residue_phase(VMEM_ROW_ADDRESS(cpu_ptr_from_dma), VMEM_ROW_ADDRESS(cpu_ptr_fft));
