...
void trigger_update_nco(const unsigned int idx);
...
static unsigned int nco_in_ptr[2];
...
void pet_fft(void) {
  ...
  if( dma_in_valid != -1 )
  {
    ...
    // {
    //     xbb_conj_multi(VMEM_ADDRESS(config_word_conj_eq_0f), 
    // VMEM_ADDRESS(cpu_ptr_from_dma), VMEM_ADDRESS(nco_in_ptr[consume_idx]), 
    // VMEM_ADDRESS(cpu_ptr_from_dma));
      
    // }
    // else if (cfo_compensation_direction == 0)
    // {
    //     xbb_conj_multi(VMEM_ADDRESS(config_word_cmul_eq_0f), 
    // VMEM_ADDRESS(cpu_ptr_from_dma), VMEM_ADDRESS(nco_in_ptr[consume_idx]), 
    // VMEM_ADDRESS(cpu_ptr_from_dma));
    // }
