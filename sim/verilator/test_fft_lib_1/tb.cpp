...
int main(int argc, char** argv, char** env) {
  ...
  // eth_node_t * eth_node = top->tb_higgs_top->eth_top->q_engine_inst->piston_inst->unode28;

  // hexdump_vmem<eth_node_t>(eth_node,"ALL",0, 1024*NSLICES / 4);
  ...
  assert( t->outs["ringbusout"]->data[t->outs["ringbusout"]->data.size()-1] == 0x0F && "One or more tests didn't pass");
