...
// allows for 4 explicit subcarriers to be put into the 0-3 index grc subcarriers
// see sjs.fixedGrc()
bool load_grc_subcarriers = true;
uint32_t grc_subcarrier_idx[4] = {2,6,10,14};
VMEM_SECTION unsigned int grc_load_mem[16];
...
void __attribute__((always_inline)) handle_grc_stream(const unsigned* const cpu_in) {
    ...
    if( ZERO_OUTPUT_OPT_0 ) {
        ...
        const uint32_t additional_tag = ZERO_OUTPUT_OPT_1;
    ...
    if( zero_stream ) {
        ...
        if( stamp_stream ) {
            ...
            const uint32_t start0  = _input_dma + grc_stream_start;
            const uint32_t length0 = OVERWRITE_CUSTOM_SC_INTO;

            const uint32_t start1  = _input_dma + custom_subcarrier_index;
            const uint32_t length1 = 1;

            const uint32_t start2  = _input_dma + grc_stream_start + OVERWRITE_CUSTOM_SC_INTO + 1;
            const uint32_t length2 = enabled_subcarriers - OVERWRITE_CUSTOM_SC_INTO - 1;


            if( load_grc_subcarriers ) {
                // load each index into a dedicated memory
                for(unsigned i = 0; i < 4; i++) {
                    grc_load_mem[i] = cpu_in[grc_subcarrier_idx[i]];
                }

                dma_block_send(VMEM_DMA_ADDRESS(grc_load_mem), 4);

                dma_block_send(start0+4, length0-4);
            } else {
...
uint32_t feedback_dengine_corrected = 1; // note that main changes this in verilator
...
unsigned do_work(
    ...
    if( unlikely( !suppress_forward && (force_forward || forward_eq) ) ) {
        ...
        if( duplex.role == DUPLEX_ROLE_RX ) {
            ...
            // FIXME: try to remove this to save cycles
            set_feedback_vector_length(&vec_eq_feedback_analog, FFT_SIZE);
...
///
/// see sjs.fixedGrc()
/// pass an index and a subcarrier
/// pass zero into index 0,1,2,3 to disable the bool
/// and disable this feature
void grc_load_callback(const unsigned data) {

    const unsigned int idx = ((data & 0x00FF0000) >> 16);
    const unsigned int sc = ((data & 0xffff));

    if( idx > 4 || sc > 1024) {
        return;
    }

    grc_subcarrier_idx[idx] = sc;

    bool found_nonzero = false;
    for(unsigned i = 0; i < 4; i++) {
        if( grc_subcarrier_idx[i] ) {
            found_nonzero = true;
            break;
        }
    }
    ...
    if( !found_nonzero ) {
        // if all zeros
        load_grc_subcarriers = false;
    } else {
        load_grc_subcarriers = true;
    }
}
...
int main2(void) {
  ...
  // setup callbacks
    ...
    ring_register_callback(&suppress_eq_callback_low, SUPRESS_EQ_FEEDBACK_L_CMD);
    ring_register_callback(&suppress_eq_callback_high, SUPRESS_EQ_FEEDBACK_H_CMD);
    ...
    ring_register_callback(&grc_load_callback, GRC_SUBCARRIER_SELECT_CMD);
