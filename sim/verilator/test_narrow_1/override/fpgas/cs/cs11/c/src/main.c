...
// was 37
#define FEEDBACK_BUS_USER_POINTERS (20)
...
#define REPEAT_INPUT (5)
#define REPEAT_BUF_SIZE (REPEAT_INPUT*FFT_SIZE) // in words
VMEM_SECTION unsigned repeat_user_data[REPEAT_BUF_SIZE];
int repeat_output_count = 40; // in frames
int repeat_active = -1;
...
static VMEM_SECTION unsigned short variable_fine_sync_eq[32];
...
uint32_t single_cooked_frame = 0;
...
static unsigned int vmem_all_pilots_full_row;
...
static uint32_t enable_composite_ul_pilot = 0; /////change from 1 to 0
...
uint32_t ook_symbol_value = 0x7fff;



// #define REPEAT_INPUT (5)
// unsigned repeat_user_data[REPEAT_INPUT];

// unsigned repeat_output_count = 40;
...
void __attribute__((optimize("Os"))) user_data_callback(
    ...
    unsigned int holding_dma = VMEM_DMA_ADDRESS(repeat_user_data);

    if( chunk_index < REPEAT_INPUT ) {
        holding_dma += (chunk_index*FFT_SIZE);

        const unsigned int holding_row = VMEM_DMA_ADDRESS_TO_ROW(holding_dma);
        const unsigned input_row = VMEM_DMA_ADDRESS_TO_ROW(dma_body);

        vmem_copy_rows(input_row, holding_row, (FFT_SIZE/16) );
    }
    ...
    switch(ud_state) {
        case UD_NOT_ACTIVE:
            ...
            if( cooked_data_type != 2 ) {
                // instantly dump
                ud_state = UD_DUMPING;
                fb_queue_ring_eth(TX_USERDATA_ERROR | ((cooked_data_type&0xffff) << 16) | 0x18);
            } else {
                // normal
                ud_state = UD_BUFFER;
            }
...
static inline bool __attribute__((always_inline)) send_cooked (
    ...
    *output_base = (uint32_t*)work_area;
    ...
    if( cooked_data_type == 1 ) {
        *input_base = (uint32_t*)vmem_all_pilots_full_row;
        *input_dma_offset = 0;
        ...
        if( cooked_data ) {
            ...
            // if this is set, we load cooked data from a separate frame
            // we also force the input offset to 0 because there are only 1024 samples
            if( single_cooked_frame ) {
                *input_dma_offset = 0;
                *input_base  = (uint32_t*)vmem_counter_snr;
            }
...
void pet_eq_and_dma_out(void) {
    ...
    uint32_t timeslot, epoc, accumulated_progress;
    ...
    const bool ud_allowed = duplex_do_userdata(&duplex, duplex_progress, false) && cooked_data_type == 2;
    ...
    // disable
    if( repeat_active >= (int32_t)repeat_output_count ) {
        repeat_active = -1;
    }

    if( repeat_every && duplex_progress == 43 ) {
        repeat_active = 0;
    }


    if( ud_allowed && (repeat_active != -1) ) {
...
// VMEM_SECTION unsigned repeat_user_data[REPEAT_BUF_SIZE];
        ...
        // annoying, but all code below expects that input_base is a cpu pointer
        if( repeat_active == 0 ) {
            input_base = (uint32_t*) (repeat_user_data);
        } else {
            const uint32_t index = ((repeat_active - 1) % 4) + 1;
            input_base = (uint32_t*) (repeat_user_data+(FFT_SIZE*index));
        }
        output_base = (uint32_t*)work_area;
...
///
    ...
    const uint32_t eq_row_one       = VMEM_ROW_ADDRESS(eq_vector_one);
    ...
    } else if ( (cooked_data_type == 2) && (duplex_mode == DUPLEX_SEND_UL_FINESYNC || duplex_mode == DUPLEX_SEND_UL_DATA ) ) {

        // we are rx node and sending S or D
        xbb_conj_multi(VMEM_ROW_ADDRESS(variable_fine_sync_eq), input_start_row, eq_row_one, work_area_row );


        // vector_multiply_1024(0, input_start_row, eq_row_one, work_area_row );
    } else {
    ...
    const uint32_t recalc_epoc = (lifetime_32 / SCHEDULE_FRAMES);
    const uint32_t recalc_accumulated_progress = (lifetime_32 % SCHEDULE_FRAMES);

    // fires 24 times per second
    if( (recalc_accumulated_progress & (1024-1)) == 0 ) {

        if( progress_report_remaining > 0 ) {
            if( recalc_accumulated_progress == 0 ) {
                ring_block_send_eth(TX_EPOC_REPORT_PCCMD | (recalc_epoc&0xffffff) );
            } else {
                ring_block_send_eth(TX_PROGRESS_REPORT_PCCMD | (recalc_accumulated_progress) );
            }
...
void fb_vector_type_callback(unsigned int *header, unsigned int *body, unsigned int vector_length, unsigned int vtype) {
    ...
    const unsigned eq_row_one = VMEM_ROW_ADDRESS(eq_vector_one);
    ...
    switch(vtype) {
...
#endif
            break;
        case FEEDBACK_VEC_TX_EQ_1:
            vmem_copy_rows(input_row,        eq_row_one, 1024/16);
            break;
        ...
        case FEEDBACK_VEC_EQ_CORRECTION: {
                ...
                vector_memory[(input_row*16)+1022] = 0x7fff;

                // only grab the first correction by looking at the value of the previous
                // one and finding a large gap
				bool accept_eq = (apply_eq_correction > 1) && ((rx_seq - previous_eq_correction_counter) > (DUPLEX_FRAMES/2));
                ...
                times_eq_corrected++; // move outside so we can count number of vectors
...
void callback_eq_setting(unsigned int data) {
    if( data == 0 ) {
        ...
        vmem_copy_rows(VMEM_ROW_ADDRESS(eq_vector_default), VMEM_ROW_ADDRESS(eq_vector_one), 1024/16);
...
uint32_t* pointer_for_generic_op(const uint32_t sel) {
    ...
    switch(sel) {
        ...
        case 1:
            ...
            break;
        case 2:
            p = (uint32_t*) &single_cooked_frame;
            break;
        case 3:
        ...
        case 8:
            ...
            break;
        case 9:
            p = (uint32_t*) &repeat_every;
            break;
        case 10:
            p = (uint32_t*) &duplex.role;
            break;
        case 11:
            p = (uint32_t*) &times_analog_eq;
            break;
        case 12:
        ...
        case 13:
            ...
            break;
        case 14:
            p = (uint32_t*) &repeat_active;
            break;
        case 15:
            p = (uint32_t*) &repeat_output_count;
            break;
        default:
            break;
...
void __attribute__((optimize("Os"))) cs20_dump_status(const unsigned int data) {
    ...
    fb_queue_ring_eth( TX_REPORT_STATUS_PCCMD | inc | (0 & 0x0000ffff)  ); inc += 0x00010000; // was epoc_needs_latch
    fb_queue_ring_eth( TX_REPORT_STATUS_PCCMD | inc | (0 & 0x0000ffff)  ); inc += 0x00010000; // was epoc_was_latched
...
void setup_pointers(void) {
    ...
    vmem_all_pilots_full_row = VMEM_ROW_ADDRESS(vmem_all_pilots_full);
    vmem_zeros_row = VMEM_ROW_ADDRESS(vmem_zeros);
}


///
/// 3 and 4 affect the same barrelshift, however 4 is the conj
/// I believe 4 is never used and is incorrect
/// 
void app_barrel_shift_callback(const unsigned int data) {
    ...
    switch(stage) {
        ...
        case 5:
            cpu_source = config_word_cmul_eq_0f;
            cpu_dest   = variable_fine_sync_eq;
            break;
        default:
...
void default_barrel_shift_callback(const unsigned int data) {
    ...
    if( is_verilator ) {
        app_barrel_shift_callback(0x020000 | 0x0f); // for verilator purpose
    } else {
    ...
    app_barrel_shift_callback(0x050000 | 0x0f);
...
void repeat_callback(const unsigned int data) {
    repeat_output_count = data & 0xffffff;
}
...
int main2(void) {
    ...
    ring_register_callback(&repeat_callback, TX_REPEAT_FRAMES_CMD);
