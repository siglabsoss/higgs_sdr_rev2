...
VMEM_SECTION unsigned short variable_uplink_eq_applied[32];
VMEM_SECTION unsigned short variable_downlink_eq_applied[32];
...
VMEM_SECTION unsigned int EQ_data_applied[1024];
VMEM_SECTION unsigned int EQ_fine_sync_data_applied[1024];
VMEM_SECTION unsigned int EQ_combined_data_applied[1024];
...
VMEM_SECTION unsigned int tx_eq_data_state[1024];
VMEM_SECTION unsigned int tx_eq_fine_sync_data_state[1024];
...
static auto_bs_t auto_initial_eq; // same as 0
uint32_t enable_auto_bs = 0xffffff; // 24 bits
...
unsigned int variable_uplink_eq_applied_row;
unsigned int variable_downlink_eq_applied_row;
...
unsigned int last_raw_finesync_report = 0x0;
...
void fine_sync_report_slow(const unsigned int v) {
    unsigned now;
    CSR_READ(TIMER_VALUE, now);

    //                       calculates a-b
    const unsigned counter_delta = subtract_timers(now,last_raw_finesync_report);

    if( counter_delta > 0x7698000 ) {
        const uint32_t low  = v & 0xffff;
        const uint32_t high = (v >> 16) & 0xffff;
        ring_block_send_eth(FINE_SYNC_RAW_PCCMD | 0x000000 | low);
        ring_block_send_eth(FINE_SYNC_RAW_PCCMD | 0x010000 | high);
        last_raw_finesync_report = now;
    }
}
...
static inline void __attribute__((always_inline)) xbb_fine_sync(
    ...
    if( enable_auto_bs ) {
        if( cooked_data_type != 2 ) {
            // in mode 0, we allow the max_freq member of the struct to control how often we run
            auto_initial_shift = run_auto_bs(&auto_initial_eq, REVERSE_VMEM_ROW_ADDRESS(input_fine_sync_location));
        } else {
            // in mode 2, we set max_freq to 0 (in the rb callback), and then control how often we run based on
            // duplex mode

            // we are tx and in P frame
            if( (duplex.role != DUPLEX_ROLE_RX) && (duplex_progress == 4)) {
                auto_initial_shift = run_auto_bs(&auto_initial_eq, REVERSE_VMEM_ROW_ADDRESS(input_fine_sync_location));
            } else if( (duplex.role == DUPLEX_ROLE_RX) && (duplex_progress == 41)) {
                // we are rx and in the first d frame
                auto_initial_shift = run_auto_bs(&auto_initial_eq, REVERSE_VMEM_ROW_ADDRESS(input_fine_sync_location));
            }
        }
    }
    ...
    if( is_verilator ) {
        vmem_copy_rows(input_fine_sync_location, input_result_location, 64);   //for verilator purpose
    } else {
    ...
    if( !dplx_do_not_rotate ) {
        ...
        if(EQ_data_rx_indi == 1) {
            ...
            if( cooked_data_type != 2)  {
                ...
                vmem_copy_rows(input_result_location, VMEM_ROW_ADDRESS(EQ_fine_sync_data_applied), 64);  


                vmem_copy_rows(VMEM_ROW_ADDRESS(EQ_data_applied), VMEM_ROW_ADDRESS(tx_eq_data_state), 64);
                vmem_copy_rows(VMEM_ROW_ADDRESS(EQ_fine_sync_data_applied), VMEM_ROW_ADDRESS(tx_eq_fine_sync_data_state), 64);
                EQ_data_rx_indi = 0;
            }else if((cooked_data_type == 2) && ((duplex_progress == DUPLEX_B0)||(duplex_progress == DUPLEX_B0+1))){
				
				if ( is_verilator ) {
            		vmem_copy_rows(input_result_location, VMEM_ROW_ADDRESS(EQ_data_applied), 64);  // for verilator purpose
				} else {
                ...
                // copy result of filter back to input
                // vmem_copy_rows(VMEM_ROW_ADDRESS(EQ_data_applied), input_result_location, 64);

                vmem_copy_rows(VMEM_ROW_ADDRESS(EQ_data_applied), VMEM_ROW_ADDRESS(tx_eq_data_state), 64);



                const unsigned input_dma = VMEM_ROW_ADDRESS_TO_DMA((VMEM_ROW_ADDRESS(EQ_data_applied)+59))+1;
                const unsigned output_dma = VMEM_ROW_ADDRESS_TO_DMA((VMEM_ROW_ADDRESS(EQ_combined_data_applied)+59))+1;
                const unsigned words = 64;
                vmem_copy_words(input_dma, output_dma, words, VMEM_ROW_ADDRESS(garbage_vmem));


            }
            else if((cooked_data_type == 2) && (duplex_progress == DUPLEX_B0+2)) { // For S
				
				if ( is_verilator ) {
           			vmem_copy_rows(input_result_location, VMEM_ROW_ADDRESS(EQ_fine_sync_data_applied), 64); // for verilator purpose
				} else {

            		 xbb_iir_one_row(VMEM_ROW_ADDRESS(config_word_cmul_eq_0f),  // cfg 1
                             VMEM_ROW_ADDRESS(config_word_add_eq_00),           // cfg 2
                             VMEM_ROW_ADDRESS(tx_eq_fine_sync_data_state),      // state
                             input_result_location,                             // measurement 
                             VMEM_ROW_ADDRESS(tx_coeff_state_one_row),          // coefficient 1
                             VMEM_ROW_ADDRESS(tx_coeff_new_measure_one_row),    // coefficient 2
                             VMEM_ROW_ADDRESS(EQ_fine_sync_data_applied));      // output
				}

 

                vmem_copy_rows(VMEM_ROW_ADDRESS(EQ_fine_sync_data_applied), 
                               VMEM_ROW_ADDRESS(tx_eq_fine_sync_data_state), 64); // can be simplifed 

                vmem_copy_rows(VMEM_ROW_ADDRESS(EQ_fine_sync_data_applied), 
                               VMEM_ROW_ADDRESS(EQ_combined_data_applied), 64);  //can be simplified
        ...
        if( cooked_data_type != 2 ) {
           ...
           // look at pilot tones, do conj multiply and then sum results
            ...
            // barrelshift 6
            xbb_conj_multi(
                variable_uplink_eq_applied_row, 
                input_result_location,
                VMEM_ROW_ADDRESS(EQ_combined_data_applied),
                input_result_location
            );
        }
    } else if(
        ...
        && (rx_eq_for_data_flag==1) ) {
           ...
           xbb_conj_multi(
                variable_downlink_eq_applied_row, 
                input_result_location,
                VMEM_ROW_ADDRESS(rx_eq_for_data),
                input_result_location
                );
   ...
   if ((((pilot_conjmul_add>>16) & 0xffff) == 0x7fff) || (((pilot_conjmul_add>>16) & 0xffff)==0x8000))
   {
    ...
    fine_sync_report_slow(pilot_conjmul_add);
 ...
 // make_nco(VMEM_DMA_ADDRESS(nco_data_for_test), 16, 0, nco_delta);
   ...
   while(1) {
	...
	vmem_copy_rows(VMEM_ADDRESS(input_result), output_fine_sync_location, 64);
}
...
void setup_barrel_shift(void) {
    ...
    variable_uplink_eq_applied_row= VMEM_ROW_ADDRESS(variable_uplink_eq_applied);
    variable_downlink_eq_applied_row= VMEM_ROW_ADDRESS(variable_downlink_eq_applied);
}
...
void app_barrel_shift_callback(const unsigned int data) {
    ...
    switch(stage) {
        ...
        case 6:
            cpu_source = config_word_conj_eq_0f;
            cpu_dest   = variable_uplink_eq_applied;
            break;
        case 7:
            cpu_source = config_word_conj_eq_0f;
            cpu_dest   = variable_downlink_eq_applied;
            break;
        default:
...
void default_barrel_shift_callback(const unsigned int data) {
    ...
    if( is_verilator ) {
        app_barrel_shift_callback(0x050000 | 0x00); // for verilator purpose
        app_barrel_shift_callback(0x060000 | 0x00); // for verilator purpose
        app_barrel_shift_callback(0x070000 | 0x00); // for verilator purpose
    } else {
        app_barrel_shift_callback(0x050000 | 0x08);
        app_barrel_shift_callback(0x060000 | 0x0b);
        app_barrel_shift_callback(0x070000 | 0x0e);
    }
}
...
void setup_auto_bs(void);

void cooked_data_type_callback(const unsigned int data) {
    cooked_data_type = data;

    if( cooked_data_type == 0 ) {
        setup_auto_bs();
    } else if( cooked_data_type == 2 ) {

        // if we are the transmitter
        // if( duplex.role != DUPLEX_ROLE_RX ) {
            auto_initial_eq.idx[0] = 985;
            auto_initial_eq.idx[1] = 985+16;
            auto_initial_eq.idx[2] = 985+32;
        // } else {
        //     auto_initial_eq.idx[0] = 6;
        //     auto_initial_eq.idx[1] = 10;
        //     auto_initial_eq.idx[2] = 126;
        // }

        // force it to run every time we call it
        auto_initial_eq.max_freq = 0;


        enable_auto_bs = 0xffffff; // reset
...
void enable_auto_bs_callback(const unsigned int data) {
    enable_auto_bs = (data &0xffffff);
}
...
int main2(void) {
    ...
    ring_register_callback(&enable_auto_bs_callback, ENABLE_AUTO_BS_CMD);
