...
unsigned int map_expected(unsigned int i) {
  unsigned int row = i % 4096;
  if(row < 2048) {
    return row;
  } else {
    return row + 2048;
...
int main(int argc, char** argv, char** env) {
  ...
  //t->inStreamAppend("cs20in", data_udp_packet() );
  t->inStreamAppend("cs20in", file_udp_packet("input.dat") );
  t->tick(140);


  // run time, 120 is enough to catch the bug
  int us = 120; 
  ...
  int output_word = 0;
  for(auto it = t->outs["cs10out"]->data.begin(); it != t->outs["cs10out"]->data.end(); it++) {
    cout << "0x" << HEX_STRING(*it) << endl;

    int got = *it;
    int expected = map_expected(output_word);

    if( got != expected ) {
        cout << "FOUND FAILURE AT word index: " << output_word << endl;
        cout << "   expected: 0x" << HEX_STRING(expected) << endl;
        cout << "   got     : 0x" << HEX_STRING(got) << endl;
        // assert fail out
        assert(got == expected);
    }
