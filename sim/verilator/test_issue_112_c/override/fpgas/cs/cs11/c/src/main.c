...
#define FEEDBACK_BUS_USER_POINTERS 37
...
uint32_t times_unpaused = 0;
uint32_t times_paused = 0;
uint32_t pause_time = 0;
bool pause_picked = false;


// difference between tx rx chain after self sync runs
// this value was hand tuned
#define TX_RX_CHAIN_FRAME_COUNTER_DELTA (6-1)
...
static unsigned int ud_frame = 0; // was ud_timeslot
static unsigned int ud_epoc = 0;
...
static unsigned int userdata_needs_fill_report = 0;
...
void __attribute__((optimize("Os"))) user_data_callback(
    ...
    switch(ud_state) {
        case UD_NOT_ACTIVE:
            ...
            last_timeslot = ud_frame = header->seq;  // timeslot
    ...
    // if( chunk_index == 1) {
    //     fb_pause_mapmov(); // hacked tb behavior
    // }
...
// hacked, left behind, but this is cs11's original function
#ifdef asdfasdf
static inline void __attribute__((always_inline)) maybe_enter_ud2(
    ...
    switch(ud_state) {
        ...
        case UD_WAIT:
            ...
            if( ud_epoc != 0 && (ud_epoc < epoc) ) {
                ...
                _puts("Epoc was in past");
            } else if( (ud_epoc == epoc) ) {
                // it is currently the epoc second
                if( (ud_frame < accumulated_progress) ) {
                    // if requested timeslot is in the past
                    ud_state = UD_DUMPING;
                    fb_queue_ring_eth(TX_USERDATA_ERROR | 7);
                } else if (ud_frame == accumulated_progress) {
                    if( can_tx ) {
                        ...
                        ud_state = UD_DUMPING;
                        fb_queue_ring_eth(TX_USERDATA_ERROR | 4);
                    }
...
// hacked, we use this version to always accept
static inline void __attribute__((always_inline)) maybe_enter_ud(
    const uint32_t progress, 
    const uint32_t timeslot,
    const uint32_t can_tx,
    const uint32_t epoc,
...
static inline void __attribute__((always_inline)) send_cooked (
...
void pet_eq_and_dma_out(void) {
    ...
    if(can_tx) {
        ...
        output_dma_offset = (0*FFT_SIZE);
        output_offset_row = VMEM_DMA_ADDRESS_TO_ROW(input_dma_offset);
    ...
    // never changes
    uint32_t eq_row_corrected = VMEM_ROW_ADDRESS(eq_vector_corrected);
    uint32_t eq_row           = VMEM_ROW_ADDRESS(eq_vector);
...
void fb_vector_type_callback(unsigned int *header, unsigned int *body, unsigned int vector_length, unsigned int vtype) {
    ...
    switch(vtype) {
...

        {
            ...
            // if( times_paused == 0 && (timer > pause_time) ) {
            //     SET_REG(x4, 0xf000);
            //     fb_pause_mapmov(); // breaking change for test 112_b
            //     SET_REG(x4, 0x0);
            //     times_paused++;
            // }
        }
    ...
    if( 1 ) {
        ...
        const uint32_t rx_seq = as_vec->seq;
        fb_queue_ring_eth(TX_PARSE_GOT_VECTOR_PCCMD | (vtype << 16) | (rx_seq&0xffff) );
    }
}
...
void cs20_reset_lifetime_32(const unsigned int data) {
    (void)data;
    lifetime_32 = 0;
    schedule->epoc_time = 0;
}

void cs20_add_lifetime_32(const unsigned int data) {
    (void)data;
...
void __attribute__((optimize("Os"))) cs20_dump_status(const unsigned int data) {
    ...
    fb_queue_ring_eth( TX_REPORT_STATUS_PCCMD | inc | (ud_frame & 0x0000ffff)  ); inc += 0x00010000;
    fb_queue_ring_eth( TX_REPORT_STATUS_PCCMD | inc | (ud_epoc & 0x0000ffff)  ); inc += 0x00010000;
    ...
    fb_queue_ring_eth( TX_REPORT_STATUS_PCCMD | inc | (userdata_needs_fill_report & 0x0000ffff)  ); inc += 0x00010000;
...
unsigned seed_set = 0;
void testbench_seed_callback(const unsigned int data) {
    seed_set = 1;
...
int main(void) {
    ...
    ring_register_callback(&testbench_seed_callback, SEED_RANDOM_CMD);
    ...
    // bool unpause_picked = false;


    // uint32_t unpause_time = 0;
    ...
    while(1) {
        ...
        // if( !unpause_picked && seed_set) {
        //     unpause_picked = true;
        //     unpause_time = 0x1000 + (simple_random()%0x7000);
        // }

        // if( !pause_picked && seed_set) {
        //     pause_picked = true;
        //     pause_time = 0x2000 + (simple_random()%0x1000);
        // }
        ...
        // if( times_paused != 0 && times_unpaused == 0 && (timer > unpause_time) ) {
        //     times_unpaused++;
        //     SET_REG(x4, 0xf001);
        //     fb_unpause_mapmov();
        //     SET_REG(x4, 0x0);
