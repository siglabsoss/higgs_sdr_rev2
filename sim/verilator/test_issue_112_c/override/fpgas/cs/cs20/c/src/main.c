...
#define INCLUDE_COUNTER_AS foo_counter
...
void build_plan(uint8_t* plan, uint32_t max) {
...
void random_stall(void) {
    uint32_t pull = (simple_random() % 1024) + 1500;
    stall2(pull);
}
...
int main(void) {
  ...
  // setup_fill_level();
    ...
    const unsigned int input_dma = VMEM_DMA_ADDRESS(foo_counter);
  ...
  CSR_WRITE(GPIO_WRITE_EN, 0xffffffff);
    ...
    uint32_t analog_len = 19;

    set_feedback_vector_length(&vec_eq_feedback_analog, analog_len);

    uint8_t plan[] = {1,2,2,2,2};

    uint32_t plan_size = ARRAY_SIZE(plan);
    ...
    while(1) {
        ...
        for(unsigned j = 0; j < 0xffffffff; j++) {
            ...
            switch(run) {
                ...
                case 1:
                    vec_eq_feedback_analog.seq = lifetime_32;
                    send_feedback_bus_header(&vec_eq_feedback_analog);
                    // if( split_send & ((simple_random() % 16) < 13) ) {
                    //     uint32_t splitlen = 6;
                    //     dma_block_send_finalized( input_dma, splitlen, 0);
                    //     stall2(1000 + (simple_random() % 2000));
                    //     dma_block_send_finalized( input_dma+splitlen, analog_len-splitlen, 1);
                    // } else {

                        dma_block_send_finalized( input_dma, analog_len, 1);
                    // }
                    break;
                case 2:
                    handle_fine_sync(foo_counter);
                    break;
                case 3:
                    random_stall();
                    break;
        ...
        dma_block_send_finalized(VMEM_DMA_ADDRESS(vmem_zeros), 16, 1);
