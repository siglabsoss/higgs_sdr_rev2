int test1(int argc, char** argv, char** env) {
    ...
    for(unsigned int i = 0; i < us; i++) {
        ...
        if( i == 40 ) {
            t->send_ring(RING_ADDR_RX_0, COOKED_DATA_TYPE_CMD | 2);
            t->send_ring(RING_ADDR_RX_1, COOKED_DATA_TYPE_CMD | 2);
            t->send_ring(RING_ADDR_RX_2, COOKED_DATA_TYPE_CMD | 2);
            t->send_ring(RING_ADDR_RX_4, COOKED_DATA_TYPE_CMD | 2);
            t->send_ring(RING_ADDR_TX_0, COOKED_DATA_TYPE_CMD | 2);
        }

        if( i == 43 ) {
            std::vector<uint32_t> pack;
            pack = op("set", 0, 3, GENERIC_OPERATOR_CMD);
