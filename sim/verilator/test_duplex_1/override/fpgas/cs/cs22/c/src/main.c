...
////////////  RX  //////////////
        ...
        if(
         ...
         else if((cooked_data_type == 2) && (duplex.role == DUPLEX_ROLE_RX) && (duplex_progress > (DUPLEX_B1+1)) && (beamformed_residue_correction_flag == 1)){
    ...
    if((cooked_data_type == 2) && (duplex.role == DUPLEX_ROLE_RX)){
	    ...
	    if(duplex_progress == DUPLEX_B1)
	    {
	    	...
	    	if(snr_subcarrier_index == 0){
	    		snr_subcarrier_index = 1;
	    	}else if(snr_subcarrier_index > 126){
	    		snr_subcarrier_index = 1;
	    	}else{
	    		snr_subcarrier_index += 2;
	    	}

	    	vector_memory[VMEM_DMA_ADDRESS(cpu_ptr_fft)+TRUNK_OFFSET+TRUNK_R1_SFO] = 0xffffff00 | snr_subcarrier_index;
	    }
