...
#include "Veth_rx_wrapper.h"
#include "Veth_rx_wrapper__Syms.h"
...
int main(int argc, char** argv, char** env) {
  ...
  Port8Out cs20out;
  cs20out.i_data = &(top->o_port_byte_3);
  cs20out.i_valid = &(top->o_port_byte_vld_3);
  cs20out.i_ready = &(top->i_port_byte_rd_3);
  // cs20out.t_last_byte = &(top->i_rx_eof);
  // cs20out.valid_meter = 0;
  // cs20out.valid_state = 0;
  // cs20out.count_up = 0;
  cs20out.random_ready = 0;
  cs20out.control_ready = false;
  ...
  for(unsigned int i = 1; /*empty*/ ; i++) {
    ...
    if ( i < 700*1000){
      if( i % 1000 == 0 ) {
        ...
        // top->i_arp_pkt_byte_rd = 0;
        // top->i_ipv4_pkt_byte_rd = 0;
      }

      if( i % 1300 == 0 ) {
            // top->i_arp_pkt_byte_rd = 1;
            // top->i_ipv4_pkt_byte_rd = 1;
          // top->arp_pkt_fifo_afull = 0;
          // top->ipv4_pkt_fifo_afull = 0;
      }      
    } else {
