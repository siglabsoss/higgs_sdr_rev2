...
int test7(int argc, char** argv, char** env) {
    ...
    // packet = file_read_hex("../../data/mapmov_320_qpsk_1_fft_1.hex");
    packet = file_read_hex("../../data/mapmov_320_qpsk_1_fft_1_randomized.hex");
    // /mnt/overflow/work/software_parent_repo/higgs_sdr_rev2/sim/data/mapmov_320_qpsk_1_fft_1_randomized.hex
...
/*
    ...
    for(const auto w : packet) {
        int16_t imag;
        int16_t real;
        ishort_to_parts(w, imag, real);
        // cout << "i: " << imag << " r: " << real << "\n";

        imag += (int16_t)(rand()%200) - 9000;
        real += (int16_t)(rand()%200) - 9000;

        uint32_t nn = parts_to_ishort(imag, real);

        // packet2.push_back()
        // cout << HEX32_STRING(w) << "\n";
        cout << HEX32_STRING(nn) << "\n";
    }
    exit(0);
*/
