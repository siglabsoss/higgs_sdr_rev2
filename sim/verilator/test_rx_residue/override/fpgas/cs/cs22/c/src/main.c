...
VMEM_SECTION unsigned int nco_data_common_phase[1024]={0};
...
void xbb_residue_phase(unsigned int input_data_location, unsigned int temp_data_location, unsigned int output_data_location)
{
   ...
   make_nco(VMEM_DMA_ADDRESS(nco_data_common_phase), 1024, nco_angle_common_phase, 0);
   ...
   xbb_conj_multi(VMEM_ADDRESS(config_word_cmul_eq_0f), input_data_location, VMEM_ADDRESS(nco_data_common_phase), temp_data_location);
...
void pet_fft() {
  ...
  if( dma_in_valid != -1 )
  {
    ...
    xbb_residue_phase(VMEM_ROW_ADDRESS(cpu_ptr_from_dma), VMEM_ROW_ADDRESS(temp_space), VMEM_ROW_ADDRESS(cpu_ptr_fft));
    ...
    if(mag_adjust_flag == 0) {
        ...
        for(int index=0; index<64; index++)   //including 16 padding
        {
            ADD_LK13(V0, V0, V2, 0);
            ADD_SK13(V1, V1, V2, 0);
        }
