...
0xffffffff
    ...
    auto fb2byte = wrapEthFeedbackPacketMulti(fb2);

    std::vector<uint32_t> zrs;
    zrs.resize(32,0);

    auto zerobytes = wrapEthFeedbackPacketMulti(zrs);
    ...
    int pull1 = 86;
    int pull2 = pull1 + rand() % 300;
    int pull3 = pull2 + rand() % 100;
    int pull4 = pull3 + rand() % 50;
    ...
    for(int i = 0; i < us; i++) {
        ...
        if( normal ) {
            ...
            if( i == pull1 ) {
                ...
                // t->inPacketAppend("ethbytein", fb2byte[0]);
                t->inPacketAppend("ethbytein", zerobytes[0]);
            }
        ...
        if( i == next_pull ) {
            ...
            bool finished = false;
            size_t this_size = (rand() % 20) + 1;

            if( first_pull ) {
                this_size += 128;
            }

            // cout << "Choosing to send " << this_size << " at " << i << '\n';

            sent_b = sent_a + this_size;

            if(sent_b >= (corrupt_packet.size()) ) {
                sent_b = corrupt_packet.size();
                finished = true;
            }

            std::vector<uint32_t> split_a(corrupt_packet.begin()+sent_a, corrupt_packet.begin() + sent_b);
            // t->inStreamAppend("cs20in", split_a);
