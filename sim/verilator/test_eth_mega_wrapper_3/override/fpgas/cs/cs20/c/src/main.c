            ...
            if( ud_epoc != 0 && (ud_epoc < epoc) ) {
                ...
                if( (ud_timeslot < timeslot) ) {
                    ...
                    if( can_tx ) {
                        ...
                        SET_REG(x3, 0x0003ffff);
                        fb_queue_ring_eth(0x0003ffff);
                        // unsigned int counter_a,counter_b;
                        // CSR_READ(TIMER_VALUE, counter_a);
                        // for(int k = 0; k < 700; k++) {
                        //     STALL(40);
                        // }
                        // CSR_READ(TIMER_VALUE, counter_b);
                        // fb_queue_ring_eth(0xD0000000 | (counter_b - counter_a));
                    } else {
