...
//     tx.set_rs(0);
    ...
    tx.set_modulation_schema(FEEDBACK_MAPMOV_QPSK);
    // tx.set_subcarrier_allocation(MAPMOV_SUBCARRIER_320);
    tx.set_subcarrier_allocation(MAPMOV_SUBCARRIER_64_LIN);
    tx.set_interleave(0);
...
int test2(int argc, char** argv, char** env) {
    ...
    unsigned illegal_types = 0;
    unsigned zeros_started = 0;
    unsigned zeros_ended = 0;

    t->registerRb([&](const uint32_t word) {
        uint32_t fb_error = 0;
        auto msg = getErrorStringFeedbackBusParse(TX_USERDATA_ERROR|word, &fb_error);
        cout << msg;
    ...
    const unsigned use_frames = 1 + (rand() % 130);
    // unsigned use_frames = 130;
    ...
    for(unsigned int i = 0; i < us; i++) {
        ...
        if( i == 100 ) {
            ...
            t->send_ring(RING_ADDR_ETH, MAPMOV_MODE_CMD | MAPMOV_SUBCARRIER_64_LIN);
        }
        ...
        if( i == 160 ) {
            ...
            // this is here to test how the test breaks
            if(0) {
    ...
    cout << "illegal_types: " << illegal_types << "\n";
    cout << "zeros_started: " << zeros_started << "\n";
    cout << "zeros_ended: " << zeros_ended << "\n";

    assert(match);
    assert(illegal_types == 0);
