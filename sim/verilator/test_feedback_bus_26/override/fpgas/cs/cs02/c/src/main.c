...
#define MODE_QPSK_64_LIN

// #define DONT_SLICE_DATA


#if defined MODE_QPSK_640_LIN || defined MODE_QAM_16_640_LIN || defined MODE_QPSK_512_LIN || defined MODE_QPSK_64_LIN
#define DISABLE_SLICE_MOVER
#endif

#ifdef DISABLE_SLICE_MOVER
#warning test_26: Reverse Mover is disabled, this linear mode
#endif

#ifdef DONT_SLICE_DATA
#warning test_26: Slicer is disabled, do not run in production
#endif

uint32_t always_slice_data = 0; // set default state of always_slice
...
bool suppress_eq_valid = false;
uint32_t suppress_feedback_until = 0;
uint32_t suppress_feedback_until_high_bits = 0;
...
// dedicated garbage used for linear mover(s)
VMEM_SECTION unsigned int garbage_mem2[16];
static unsigned int garbage_mem2_row;


#include "cs21_example_data.h"


static unsigned int enabled_subcarriers; // delcared here but SET BY OUTPUT FROM schedule_maker.py
static unsigned int number_active_schedules; // same as previous
#ifndef DISABLE_SLICE_MOVER
static unsigned int enabled_subcarriers_data; // delcared here but SET BY OUTPUT FROM schedule_maker.py
static unsigned int number_active_schedules_data; // same as previous
#endif
...
static void mover_64_lin(const unsigned int input_row, const unsigned int output_row) {
    // we are moving from row 59
    // this is because on the tx side we are using row 1 length of 4 rows
    // so we move from rows 64 - 1 - 4
    // however we also need off by 1 word due to tx/rx flip due to ifft ifft
    const unsigned input_dma = (VMEM_ROW_ADDRESS_TO_DMA((input_row+59))) + 1;
    const unsigned output_dma = VMEM_ROW_ADDRESS_TO_DMA(output_row);
    const unsigned words = 64;

    // also pass garbage row, I made garbage_mem2 dedicated garbage for linear movers
    vmem_copy_words(input_dma, output_dma, words, garbage_mem2_row);

    // row version does not work
    // vmem_copy_rows(input_row + 59, output_row, 4);
}
...
void __attribute__((always_inline)) handle_sliced_data(const unsigned index) {
    ...
    if( index==0 ) {
        input_row = VMEM_ROW_ADDRESS((input_dma_buf));
        output_row = VMEM_ROW_ADDRESS(dst_mem_data_a);
    } else {
        input_row = VMEM_ROW_ADDRESS((input_dma_buf + PING_PONG_BUFFER_SIZE_IN));
        output_row = VMEM_ROW_ADDRESS(dst_mem_data_b);
    }

#endif

    #ifdef MODE_QPSK_640_LIN
    mover_640_lin(input_row, output_row);
    #endif
    #ifdef MODE_QAM_16_640_LIN
    mover_640_lin(input_row, output_row);
    #endif
    #ifdef MODE_QPSK_512_LIN
    mover_512_lin(input_row, output_row);
    #endif
    #ifdef MODE_QPSK_64_LIN
    mover_64_lin(input_row, output_row);
    #endif
    ...
    #ifdef MODE_QPSK_64_LIN
    demod_body_length = 4;        // How many words is the fb bus after slice
    slice_input_length = 64;      // How many subcarriers before slicing
    #endif
...
uint32_t debug_force_forward = 0;
...
unsigned do_work(
    ...
    const bool force_forward = (debug_force_forward > 0) && (duplex_progress==0);

    // if suppress is valid, we suppress until lifetime is larger than the value passed
    const bool suppress_forward = suppress_eq_valid ? (lifetime_32 < suppress_feedback_until) : false;

    if( unlikely( !suppress_forward && (force_forward || forward_eq) ) ) {
    
        if( force_forward ) {
            debug_force_forward--;
        }

        if( suppress_eq_valid ) {
            // if this was true, it means we were supressing
            // and this is the first eq after supression
            suppress_eq_valid = false;
        }
...
uint32_t suppress_eq_valid_copy;
...
uint32_t* pointer_for_generic_op(const uint32_t sel) {
    ...
    switch(sel) {
        ...
        case 7:
            ...
            break;
        case 8:
            p = &debug_force_forward;
            break;
        case 10:
            p = (uint32_t*) &duplex.role;
            break;
        case 11:
            p = &suppress_feedback_until;
            break;
        case 12:
            suppress_eq_valid = suppress_eq_valid_copy;
            p = &suppress_eq_valid_copy;
            break;
...
void setup_pointers(void) {
    garbage_mem2_row = VMEM_ROW_ADDRESS(garbage_mem2);
}

static void set_demod_mode(void) {
...
void suppress_eq_callback_low(const unsigned data) {
    suppress_eq_valid = true;
    suppress_feedback_until = suppress_feedback_until_high_bits | (data&0xffffff);
    // FIXME check that high bits are ok 
}

void suppress_eq_callback_high(const unsigned data) {
    // FIXME load high bits
    // FIXME check this works
    suppress_feedback_until_high_bits = (data & 0xff) << 24;
}
