...
int main(int argc, char** argv, char** env) {
  ...
  // you can also insert for check streams from those functins()
    ...
    auto schedule_packet = feedback_vector_packet(
  ...
  unsigned int next_pull = us_base + rand() % pull_gap_maximum_us;
    ...
    unsigned int pull_count_limit = 3;
  ...
  if(next_pull > us) {
    ...
    bool do_inside_sabotage = false; //corruption_mode == 1 || corruption_mode == 2;
    ...
    bool do_mapmov_sabotage = true;

    std::vector<uint32_t> data;
    dataSync(&data, 0xdeadbeef, 128);
    dataSync(&data, 0xcafebabe, 128);
    ...
    unsigned pop_amount = 1+(rand() % (data.size()-1));

    cout << "not sending final " << pop_amount << " words of the message" << endl;

    if( back ) {
        for(auto j = 0; j < pop_amount; j++ ) {
            data.pop_back();
        }
    } else {
    ...
    t->inStreamAppend("cs20in", packet);
     ...
     bool do_outside_sabotage = true; //corruption_mode == 0 || corruption_mode == 2;

     // perfectly good code for doing an outside jam
    if(do_outside_sabotage) {
        // randomly jam
        uint32_t jam_amount = (rand() % 400) + 1;
        // uint32_t jam_with = 3;
        ...
        for(auto i = 0; i < jam_amount; i++) {
            do_jam.push_back(rand() & 0xffffffff);
        }
    ...
    t->inStreamAppend("ringbusin", ringbus_udp_packet(RING_ADDR_CS20, MAPMOV_RESET_CMD) );
    ...
    for(unsigned int i = 0; i < recover_us; i++) {
        ...
        if( i % 5 == 0 && i < 50 ) {
            t->inStreamAppend("cs20in", feedback_flush_packet( 10 ) );
        }
        ...
        if( i == 72 ) {
            t->inStreamAppend("cs20in", schedule_packet);
    ...
    if(!found3) {
        cout << "found3 was NOT set, something is wrong" << endl;
    } else {
        cout << "found3 was set" << endl;
    }
