...
int test5(int argc, char** argv, char** env) {
    ...
    t->ins["cs31in"]->respect_ready = true;
    ...
    // packet = file_read_hex("../../data/mapmov_320_qpsk_1_fft.hex");
    ...
    // packet = gain_ishort_vector(0.1, packet, saturations);
    ...
    dumpVector32("sliced_out.hex", sliced_out);
    ...
    const auto expected = file_read_hex("expected_sliced.hex");

    const bool same = expected == sliced_out;
