...
int main(int argc, char** argv, char** env) {
  ...
  std::cout << "Ring got out " << t->outs["ringbusout"]->data.size()
            << " items\n";
  ...
  for(auto fpga = RING_ENUM_CS11; fpga <= RING_ENUM_CS20; fpga++) {
    for(auto data = 0; data < 32; data++) {
      expected.push_back( fpga | (0xE00+(data*0x10)) );
    }
  }

  std::cout << "Expected " << expected.size() << " items\n";
