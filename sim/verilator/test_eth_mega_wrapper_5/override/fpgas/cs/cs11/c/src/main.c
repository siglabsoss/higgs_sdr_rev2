...
#define FEEDBACK_BUS_USER_POINTERS 45
...
#define TX_RX_CHAIN_FRAME_COUNTER_DELTA (2+4)
...
static VMEM_SECTION unsigned int nco_data[1024] = {0};
...
static VMEM_SECTION unsigned int work_area[1024*5];
...
static inline void __attribute__((always_inline)) send_cooked (
    ...
    const uint32_t duplex_progress,
    uint32_t**  input_base,
    ...
    *output_base = (uint32_t*)work_area;
    ...
    if( cooked_data_type == 1 ) {
        ...
        *input_dma_offset = 0;
        ...
        const uint32_t duplex_mode = get_duplex_mode(&duplex, duplex_progress);

        bool eq_frame = false;
        bool cooked_data = false;

        switch(duplex_mode) {
            case DUPLEX_SEND_DL_PILOT:
                eq_frame = true;
                break;
...
void pet_eq_and_dma_out(void) {
    ...
    uint32_t duplex_progress = lifetime_32 % DUPLEX_FRAMES;
    ...
    // if( self_sync_state == 0 ) {
        dma_block_send(work_area_dma, 1024);
    // } else {
    //     // schedule timer for the future
    //     dma_block_send_timer(work_area_dma, 1024, self_sync_timer);

    //     // do calculations
    //     const unsigned hi = (lifetime_32>>16) & 0xffff;
    //     const unsigned low = lifetime_32 & 0xffff;
    ...
    //         delta = now - self_sync_timer;
    //         SET_REG(x3, delta);
    //     } while(delta < 0);

    //     self_sync_state = 0;

    //     ring_block_send_eth(SELF_SYNC_REPLY_PCCMD | 0x00000 | low);
    //     ring_block_send_eth(SELF_SYNC_REPLY_PCCMD | 0x10000 | hi);
    // }
...
int main2(void) {
    ...
    dma_block_send(VMEM_DMA_ADDRESS(_memory_manager_chunks), 2048);
