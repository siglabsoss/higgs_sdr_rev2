...
void pet_mover(void) {
  ...
  if(dma_in_full && outgoing_dma_occupancy == 0) {
    ...
    lifetime_32 = vector_memory[dma_idx_to_ptr(dma_trig_previous) +
                                            FFT_SIZE +
                                            TRUNK_FRAME_COUNTER];
    ...
    if( stamp_stream ) {
        dma_block_send_finalized(VMEM_DMA_ADDRESS(stamped_default_stream), enabled_subcarriers*FRAME_MOVE_CHUNK, 1);
    } else {
