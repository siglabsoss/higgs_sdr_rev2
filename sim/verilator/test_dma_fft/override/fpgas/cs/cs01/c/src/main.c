...
void dma_out_set_safe(unsigned int dma_ptr);
...
void pet_fft(void) {
  ...
  static unsigned int ring_counter = 0;
  ...
  if( dma_in_valid != -1 )
  {
    ...
    // if( ring_counter == 50000 ) {
      unsigned int check = cpu_ptr_from_dma[111];
      if( check == 0 ) {
        ring_block_send_eth(0x0c000000 | 1);
      } else {
        // ring_block_send_eth(0x0c000000 | 2);
      }
      ring_counter = 0;
    // }
    ring_counter++;
...
void dma_out_set_safe(unsigned int dma_ptr) {
  ...
  dma_out_set(dma_ptr, DMA_IN_CHUNK);
}

// set in cirbuf if this is the "cyclic postfix" dma
#define IS_CP_MASK (0x2)
...
void pet_dma_out(void) {
  ...
  if( (!output_blocked) && (fft_valid != -1) ) {
    ...
    // note I am setting CP flag on second one, so more like "Cyclic postfix"


    error = circular_buf_put(&dma_out_started, consume_idx); MY_ASSERT(error == 0);
    error = circular_buf_put(&dma_out_started, consume_idx | IS_CP_MASK); MY_ASSERT(error == 0);

    // CSR_READ(DMA_1_SCHEDULE_OCCUPANCY, dma_occupancy);
    // CSR_READ(DMA_1_STATUS, dma_occupancy_status);
    // dma_occupancy_combined = dma_occupancy + dma_occupancy_status;
    // CSR_WRITE(GPIO_WRITE, (0x5a<<8) | dma_occupancy_combined);


    // similar to just calling dma_out_set
    // schedule two full fft outputs (CP)
    dma_out_set_safe(VMEM_DMA_ADDRESS(cpu_ptr_from_fft));

    // CSR_READ(DMA_1_SCHEDULE_OCCUPANCY, dma_occupancy);
    // CSR_READ(DMA_1_STATUS, dma_occupancy_status);
    // dma_occupancy_combined = dma_occupancy + dma_occupancy_status;
    // CSR_WRITE(GPIO_WRITE, (0x5b<<8) | dma_occupancy_combined);
    ...
    // CSR_READ(DMA_1_STATUS, dma_occupancy_status);
    // dma_occupancy_combined = dma_occupancy + dma_occupancy_status;
    // CSR_WRITE(GPIO_WRITE, (0x5c<<8) | dma_occupancy_combined);
  ...
  // if the value has the IS_CP_MASK set, then we know it's time to go
  // CSR_READ(mip, helper);
  CSR_READ(DMA_1_SCHEDULE_OCCUPANCY, dma_occupancy);
