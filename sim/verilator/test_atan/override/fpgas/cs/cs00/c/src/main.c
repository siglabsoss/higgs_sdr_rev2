...
unsigned int enable_output_stream;
unsigned int pending_input_advance;
unsigned int sync_finished = 0;
...
void pet_fsm(void) {
    ...
    switch(fsm_state) {
        case FSM_FLUSHING:
            ...
            dma_out_occupancy = circular_buf_occupancy(&dma_out_started);
...
void xbb_coarse_sync(CFO_Results *CFO_results, unsigned int coarse_sync_number) 
{
  ...
  ring_block_send_eth(0xdead0001);
  ...
  unsigned int exp_data_1280_ext_location=VMEM_ROW_ADDRESS(exp_data_1280_ext);
  ...
  unsigned int occupancy;
  unsigned int expected_occupancy=2;
  ...
  for(int index=0; index<coarse_sync_number; index++)
  {
    ...
    // CSR_READ(TIMER_VALUE, start_clk);
    xbb_conj_multi(VMEM_ROW_ADDRESS(config_word_conj_eq_0f), input_conj_multi_location_0, input_conj_multi_location_1, output_conj_multi_location);
  ...
  xbb_cfo_estimation(CFO_results, VMEM_ROW_ADDRESS(config_word_conj_eq_0b), VMEM_ROW_ADDRESS(config_word_cmul_rx4_0f), sample_cp_location, sample_cp_location_1024, all_one_location, output_conj_multi_location, row_num_cfo, bank_address_onetone_calculation_location, permutation_address_onetone_calculation_location);
  ...
  // ring_block_send_eth(sample_cp_advance);
  dma_in_set(dma_in_address,  sample_cp_advance+sample_cp_advance_row_adjustment*16);

  // call this function instead of doing DMA
  // turnstile_advance(sample_cp_advance);
...
void pet_symbol_sync() {
    if( fsm_state == FSM_DO_SYNC ) {
        xbb_coarse_sync(&cfo_run_results, 15);
        sync_finished = 1;
    }
