...
// void send_ring_heartbeat(int value) {
//     CSR_WRITE(RINGBUS_WRITE_ADDR, ETH_RING_ENUM);
//     CSR_WRITE(RINGBUS_WRITE_DATA, (TEST_CMD | (value&DATA_MASK)));
...
// unsigned int dma_out_expected_occupancy = 0;

unsigned int fake_work_done = 0;
...
void pet_dma_out() {
  ...
  if( incomming_occupancy != 0 ) {
    ...
    if( fake_work_done % 10 == 0 ) {
      work_done = 1;
    }
    fake_work_done++;

    // if output can accept more
    if(work_done && (dma_occupancy < DMA_1_SCHEDULE_DEPTH) )  {
      ...
      trig_dma_out(dma_idx_to_ptr(data));
  ...
  CSR_READ(DMA_1_SCHEDULE_OCCUPANCY, dma_occupancy); // RE - READ !
  unsigned int outgoing_occupancy = circular_buf_occupancy(&dma_schedule_out);

  // if something finished
  if( dma_occupancy != outgoing_occupancy ) {
    int dma_just_finished;
    error = circular_buf_get(&dma_schedule_out, &dma_just_finished); MY_ASSERT(error == 0);

    // fixme how to report to upstream that dma_just_finished is freed up to be overwritten

  }
}

#define FILL_LEVEL_REPORT_TIME (1000)
...
int main(void)
{
  ...
  unsigned int report_timer, report_timer_check;
  ...
  while(1) {
    ...
    if( report_timer_check >= (report_timer+FILL_LEVEL_REPORT_TIME) ) {
      ring_block_send_eth(circular_buf_occupancy(&dma_in_buffer));
    ...
    if (fsm_state==MAP_STATE) {
      ...
      for (unsigned int i = 0; i < 16; i++){
          ...
          mapper_load_qpsk(&qpsk_table, ( (i*NSLICES) + (MAPPER_DEST_ROW*NSLICES) ), i+input_dma_ptr);
      }
    ...
    if (fsm_state==TX_STATE){
      ...
      dma_out_set(DST_ROW*NSLICES, FFT_SIZE*32);
    ...
    switch(fsm_state) {
       ...
       case WAITING_STATE:
           // return_time = dma_in_check(3);

           // CSR_WRITE(GPIO_WRITE, (0x200000) | 4);
           
           // // send_ring_heartbeat(return_time);
