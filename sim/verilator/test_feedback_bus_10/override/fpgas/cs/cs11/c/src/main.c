...
void user_data_callback(
    ...
    if(insert_error != 0) {
        fb_queue_ring_eth(CS11_USERDATA_ERROR | 8);
    }
...
void pet_eq_and_dma_out(void) {
    ...
    uint32_t frame_phase = (progress % 5);
    uint32_t input_frame_phase = (progress & (16-1) ); // same as % 16
    ...
    if(userdata_needs_fill_report) {
        ...
        fb_queue_ring_eth(CS11_FILL_LEVEL_PCCMD | masked );
    }
        ...
        case UD_WAIT:
            ...
            if( ud_epoc != 0 && (ud_epoc < epoc) ) {
                ...
                if( (ud_timeslot < timeslot) ) {
                    ...
                    ud_state = UD_DUMPING;
                    fb_queue_ring_eth(CS11_USERDATA_ERROR | 5);
                }
    ...
    if(can_tx) {
        if(
            ...
            // ring_block_send_eth(0xface0000|pending_data);
            /*if(pending_frame<5){
                // SET_REG(x3, 0xCDCDCDCD);
                // SET_REG(x3, input_dma_offset);
                // SET_REG(x3, input_offset_row);
            }
            else {
               pending_frame = 1;
    ...
    if( (accumulated_progress & (1024-1)) == 0 ) {
        ...
        if( progress_report_remaining > 0 ) {
            if( accumulated_progress == 0 ) {
                ring_block_send_eth(CS11_EPOC_REPORT_PCCMD | (epoc&0xffffff) );
            } else {
                ring_block_send_eth(CS11_PROGRESS_REPORT_PCCMD | (accumulated_progress) );
            }
            progress_report_remaining--;
        }
    }

    // SET_REG(x4, 0xdeadba00);
    // SET_REG(x4, epoc);
    // SET_REG(x4, 0xdeadba01);
    // SET_REG(x4, accumulated_progress);
...
void fb_vector_type_callback(unsigned int *header, unsigned int *body, unsigned int vector_length, unsigned int vtype) {
    ...
    fb_queue_ring_eth(0xdead0000 | header[0] << 8 | header[4]);
    fb_queue_ring_eth(body[0]);
    switch(vtype) {
...
void cs20_add_lifetime_32(unsigned int data) {
    ...
    SET_REG(x4, 0);
    SET_REG(x4, 0xdeadbabe);
    SET_REG(x4, schedule->epoc_time);
...
void __attribute__((optimize("Os"))) cs20_dump_status(unsigned int data) {
    int inc = 0x00000000;

    fb_queue_ring_eth( CS11_REPORT_STATUS_PCCMD | inc | (frame_counter & 0x0000ffff)  ); inc += 0x00010000;
    fb_queue_ring_eth( CS11_REPORT_STATUS_PCCMD | inc | (lifetime_frame_counter & 0x0000ffff)  ); inc += 0x00010000;
    fb_queue_ring_eth( CS11_REPORT_STATUS_PCCMD | inc | (lifetime_32 & 0x0000ffff)  ); inc += 0x00010000;
    fb_queue_ring_eth( CS11_REPORT_STATUS_PCCMD | inc | ((lifetime_32>>16) & 0x0000ffff)  ); inc += 0x00010000;
    fb_queue_ring_eth( CS11_REPORT_STATUS_PCCMD | inc | (pending_data & 0x0000ffff)  ); inc += 0x00010000;
    fb_queue_ring_eth( CS11_REPORT_STATUS_PCCMD | inc | (pending_timeslot & 0x0000ffff)  ); inc += 0x00010000;
    fb_queue_ring_eth( CS11_REPORT_STATUS_PCCMD | inc | (pending_length & 0x0000ffff)  ); inc += 0x00010000;
    fb_queue_ring_eth( CS11_REPORT_STATUS_PCCMD | inc | (output_frame_count & 0x0000ffff)  ); inc += 0x00010000;
    fb_queue_ring_eth( CS11_REPORT_STATUS_PCCMD | inc | (epoc_needs_latch & 0x0000ffff)  ); inc += 0x00010000;
    fb_queue_ring_eth( CS11_REPORT_STATUS_PCCMD | inc | (epoc_was_latched & 0x0000ffff)  ); inc += 0x00010000;
    fb_queue_ring_eth( CS11_REPORT_STATUS_PCCMD | inc | (progress_report_remaining & 0x0000ffff)  ); inc += 0x00010000;
    fb_queue_ring_eth( CS11_REPORT_STATUS_PCCMD | inc | (user_callbacks & 0x0000ffff)  ); inc += 0x00010000;
    fb_queue_ring_eth( CS11_REPORT_STATUS_PCCMD | inc | (ud_state & 0x0000ffff)  ); inc += 0x00010000;
    fb_queue_ring_eth( CS11_REPORT_STATUS_PCCMD | inc | (ud_timeslot & 0x0000ffff)  ); inc += 0x00010000;
    fb_queue_ring_eth( CS11_REPORT_STATUS_PCCMD | inc | (ud_epoc & 0x0000ffff)  ); inc += 0x00010000;
    fb_queue_ring_eth( CS11_REPORT_STATUS_PCCMD | inc | (ud_total_chunks & 0x0000ffff)  ); inc += 0x00010000;
    fb_queue_ring_eth( CS11_REPORT_STATUS_PCCMD | inc | (ud_chunk_index_consumed & 0x0000ffff)  ); inc += 0x00010000;
    fb_queue_ring_eth( CS11_REPORT_STATUS_PCCMD | inc | (ud_chunk_length & 0x0000ffff)  ); inc += 0x00010000;
    fb_queue_ring_eth( CS11_REPORT_STATUS_PCCMD | inc | (ud_asked_for_pause & 0x0000ffff)  ); inc += 0x00010000;
    fb_queue_ring_eth( CS11_REPORT_STATUS_PCCMD | inc | (pending_lifetime_update & 0x0000ffff)  ); inc += 0x00010000;
    fb_queue_ring_eth( CS11_REPORT_STATUS_PCCMD | inc | (incoming_future_lifetime_counter & 0x0000ffff)  ); inc += 0x00010000;
    fb_queue_ring_eth( CS11_REPORT_STATUS_PCCMD | inc | (userdata_needs_fill_report & 0x0000ffff)  ); inc += 0x00010000;
    fb_queue_ring_eth( CS11_REPORT_STATUS_PCCMD | inc | (run_sfo & 0x0000ffff)  ); inc += 0x00010000;
    fb_queue_ring_eth( CS11_REPORT_STATUS_PCCMD | inc | (tdma_mode & 0x0000ffff)  ); inc += 0x00010000;
    fb_queue_ring_eth( CS11_REPORT_STATUS_PCCMD | inc | (tdma_mode_pending & 0x0000ffff)  ); inc += 0x00010000;
}
...
int main(void)
{
    ...
    schedule->epoc_time = 2100;
    lifetime_32 =  6144-8-4-4;
    // // SET_REG(x4, 0);
    ...
    ring_register_callback(&cs20_control_progress_report, CS11_PROGRESS_CMD);
    ring_register_callback(&cs20_dump_status, CS11_MAIN_REPORT_STATUS_CMD);
...
#ifdef DEBUG_TDMA_VALUES
        if(counter > 100000) {
            // ring_block_send_eth(TDMA_REPLY_PCCMD | 0);
            // ring_block_send_eth(TDMA_REPLY_PCCMD | tdma_mode);
            // ring_block_send_eth(TDMA_REPLY_PCCMD | tdma_mode_pending);
            counter = 0;
        }
        counter++;
#endif
