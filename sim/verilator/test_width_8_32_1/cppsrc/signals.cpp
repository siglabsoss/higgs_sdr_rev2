...
void signals::init_top() {
  ...
  top = new Vwidth_8_32; // Or use a const unique_ptr, or the VL_UNIQUE_PTR wrapper
...
uint32_t signals::reset(uint32_t val) {
  top->reset = val;
  return top->reset;
}
...
uint32_t signals::fillcount() {
  return top->fillcount;
}
...
Napi::Number signals::resetWrapped(const Napi::CallbackInfo& info) {
  Napi::Env env = info.Env();
  if(info.Length() > 1 || (info.Length() == 1 && !info[0].IsNumber())) {
    Napi::TypeError::New(env, "Number expected").ThrowAsJavaScriptException();
  }
  ...
  if(info.Length() == 1) {
    Napi::Number val = info[0].As<Napi::Number>();
    returnValue = Napi::Number::New(env, signals::reset(val.Int32Value()));
  } else {
    returnValue = Napi::Number::New(env, top->reset);
  }
  return returnValue;
}
...
Napi::Number signals::fillcountWrapped(const Napi::CallbackInfo& info) {
  Napi::Env env = info.Env();
  if(info.Length() > 0) {
    Napi::TypeError::New(env, "Number expected").ThrowAsJavaScriptException();
  }
  ...
  Napi::Number returnValue;
  returnValue = Napi::Number::New(env, top->fillcount);
  return returnValue;
}
...
Napi::Object signals::Init(Napi::Env env, Napi::Object exports) {
  ...
  exports.Set("reset", Napi::Function::New(env, signals::resetWrapped));
  ...
  exports.Set("fillcount", Napi::Function::New(env, signals::fillcountWrapped));
