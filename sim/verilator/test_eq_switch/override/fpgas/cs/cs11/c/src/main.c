...
unsigned tdma_subcarrier = 17;
unsigned int lifetime_32 = 0; // for tdma
schedule_t _schedule;
schedule_t* schedule = &_schedule;

#include "tx_tdma.h"

unsigned int lifetime_frame_counter = 0; // for sfo


unsigned int pending_epoc_update = 0;
...
uint32_t mask_sc[MAXIMUM_MASK_SC];
uint32_t mask_sc_enabled = 0;
...
void check_subcarrier(unsigned int* data) {
    ...
    if( mag2 < tol ) {
        ring_block_send_eth(CS10_MAG_CHECK_PCCMD | 0x20000 | ((uint32_t)(real&0xffff)) );
        ring_block_send_eth(CS10_MAG_CHECK_PCCMD | 0x30000 | ((uint32_t)(imag&0xffff)) );
    }
}
...
// #ifdef OVERWRITE_COUNTER
    ...
    if(progress == 0 && timeslot == 0 && tdma_mode_pending != 0) {
        ...
        tdma_argument = tdma_argument_pending;
    }

if(can_tx) {

    // only insert when tdma_mode less than or equal to this threshold
    if( tdma_mode <= TDMA_INSERT_MODE_MAXIMUM ) {
        // mask first
        mask_subcarriers(input_dma_start);

        // write our data second
...
void fb_vector_type_callback(unsigned int *header, unsigned int *body, unsigned int vector_length, unsigned int vtype) {
    ...
    switch(vtype) {
		...
		if(body[i] != i) {
		    fb_queue_ring_eth(DEBUG_0_PCCMD | 1);
		    break;
		}
	    }
	    fb_queue_ring_eth(DEBUG_0_PCCMD | 0);
	    break;
        default:
            fb_vector_default();
            break;
...
uint32_t debug_generic_op = 0x42;
void handle_generic_callback(unsigned int data) {
    ...
    switch(sel) {
        ...
        case 1:
            ...
            break;
        // disable this case, so we will hit default and RETURN to avoid zero pointer weirdness
        // case 2:
        //     p = (uint32_t*) &schedule->offset;
            // break;
        case 3:
            p = (uint32_t*) &schedule->epoc_time;
            break;
        case 4:
            p = (uint32_t*) &tdma_subcarrier;
            break;
        case 5:
    ...
    switch(op) {
        ...
        case 4:
            ...
            fb_queue_ring_eth(GENERIC_READBACK_PCCMD |           (op_sel<<16) | capture_low);
            fb_queue_ring_eth(GENERIC_READBACK_PCCMD | 0x80000 | (op_sel<<16) | capture_high);
            break;
    }
}
...
// unsigned int eq_hash_state = 0;
    ...
    if( go ) {
        ...
        for(int i = 0; i < 16; i++) {
            ...
            fb_queue_ring_eth(CS20_USERDATA_ERROR | ((word & 0xffff0000)>>8) | 0x16 );
            fb_queue_ring_eth(CS20_USERDATA_ERROR | ((word & 0xffff)<<8)     | 0x17 );
        }
#endif
...
void set_tdma_sc_callback(const unsigned int data) {
    if( data < 1024 ) {
        tdma_subcarrier = data;
    } else {
        // error, do nothing
    }
}
...
void set_mask_sc_callback(const unsigned int data) {
    uint32_t dmode = (data >> 16) & 0xff; // 8 bits
    uint32_t data_16 = data & 0xffff; // 16 bits
...
int main(void)
{
    ...
    ring_register_callback(&handle_generic_callback, GENERIC_OPERATOR_CMD);
    ...
    ring_register_callback(&set_tdma_sc_callback, SET_TDMA_SC_CMD);
