...
void test0(int argc, char** argv, char** env) {
    ...
    auto default_eq = file_read_hex("../../../libs/s-modem/soapy/js/test/data/default_eq_packet.hex");


    auto rb_packet = file_read_hex_stream("../../data/ringbus1.hexstream");

    auto mapmov0 = file_read_hex_stream("../../data/mapmov0.hexstream");
    auto mapmov1 = file_read_hex_stream("../../data/mapmov1.hexstream");


    std::size_t const half_size = 367;
    ...
    auto build_mapmov0 = wrapEthFeedbackPacket(split_a);


    std::vector<uint32_t> exshort({
0x00000002,
0x00018010,
0x00000000,
0x80000000,
...
0xffffffff
    ...
    auto tx13 = file_read_hex("../../../libs/s-modem/soapy/js/test/data/nuc_tx_13.hex");

    tx13.erase(tx13.begin(), tx13.begin()+10400);

    auto fb2byte = wrapEthFeedbackPacketMulti(tx13);

    cout << "split into " << fb2byte.size() << " packets\n";
    ...
    auto eqbyte = wrapEthFeedbackPacketMulti(default_eq);

    // cout << eqbyte.size() << endl;
    // exit(0);



    // for(int i = 0; i < 400; i++) {
    //     cout << HEX_STRING(int(mapmov0[i])) << " - " << HEX_STRING(int(build_mapmov0[i])) << endl;
    // }
    // exit(0);

    // rb_packet.insert(rb_packet.begin(), 0);



    // for( auto x : rb_packet ) {
    //     cout << HEX_STRING((int)x) << endl;
    // }

    // corrupt_packet.resize(380);
    ...
    next_pull = pull1;
    // int pull2 = pull1 + rand() % 6;
    // int pull3 = pull2 + rand() % 6;
    // int pull4 = pull3 + rand() % 6;

//     if( pull2 > 1980 || pull3 > 1980 || pull4 > 1980  ) {
//         pull2 = 1950;
//         pull3 = 1951 + rand() % 5;
//         pull4 = 1960 + rand() % 5;
//     }
...
// cout << "pull1 " << pull1 << endl;
// cout << "pull2 " << pull2 << endl;
// cout << "pull3 " << pull3 << endl;
// cout << "pull4 " << pull4 << endl;
    ...
    for(int i = 0; i < us*500; i++) {
        ...
        if( normal ) {
            ...
            if( i == pull1 || i == next_pull ) {
                 // t->inPacketAppend("ethbytein", mapmov0);
                // t->inPacketAppend("ethbytein", build_mapmov0);
                
                // for(int j = 0; j < fb2byte.size(); j++ ) {
                //     t->inPacketAppend("ethbytein", fb2byte[j]);
                //     packets_sent++;
                // }

                if( packets_sent < fb2byte.size() ) {
                    cout << "sent packet " << packets_sent << " at " << i << endl;

                    t->inPacketAppend("ethbytein", fb2byte[packets_sent]);
                    packets_sent++;

                    next_pull = i + 1 + rand() % (13*500);
                }
            ...
            // if( i == pull2 ) {
            //     // t->inPacketAppend("ethbytein", mapmov1);
            //     t->inPacketAppend("ethbytein", fb2byte[1]);
            // }

            // if( i == pull3 ) {
            //     // t->inPacketAppend("ethbytein", mapmov1);
            //     t->inPacketAppend("ethbytein", fb2byte[2]);
            // }

            // if( i == pull4 ) {
            //     // t->inPacketAppend("ethbytein", mapmov1);
            //     t->inPacketAppend("ethbytein", fb2byte[3]);
            // }

            // if( i == 2000 || i == 2002 ) {
            //     // t->inPacketAppend("ethbytein", wrapEthFeedbackPacket(default_eq));
            //     t->inPacketAppend("ethbytein", eqbyte[0]);
            //     t->inPacketAppend("ethbytein", eqbyte[1]);
            //     t->inPacketAppend("ethbytein", eqbyte[2]);
            // }
        } else {
    ...
    for(auto it = t->outs["ringbusout"]->data.begin(); it != t->outs["ringbusout"]->data.end(); it++) {
        ...
        switch(cmd) {
            case CS20_USERDATA_ERROR: {
                auto msg = getErrorStringFeedbackBusParse(*it, &critical_error);
                cout << msg;
    ...
    auto expected_cs20 = file_read_hex("cs20_in_expected.hex");

    bool data_correct = false;

    if( expected_cs20 == t->monitors["mcs20in"]->data ) {
        data_correct = true;
    }

    cout << "expected: " << endl;
    // for(auto w: expected_cs20) {
    //     cout << HEX32_STRING(w);
    // }

    // cout << "got: " << endl;
    // for(auto w: t->monitors["mcs20in"]->data) {
    //     cout << HEX32_STRING(w);
    // }
    ...
    bool pass = (crit_fail == false) && data_correct;
