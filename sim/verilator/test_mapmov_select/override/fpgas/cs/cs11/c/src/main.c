...
void user_data_callback( 
    ...
    // ring_block_send_eth(cpu_body)

    // values under 32 are map-moved
    if(
        cpu_body[33] == 33 &&
        cpu_body[1023] == 1023) {
        passed_checks++;
    } else {
        ring_block_send_eth(DEBUG_3_PCCMD);  // fail
    }

    if( passed_checks == (0x1f+1) ) {
        ring_block_send_eth(DEBUG_4_PCCMD); // pass
    }
