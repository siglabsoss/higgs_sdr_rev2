...
void test0(int argc, char** argv, char** env) {
    ...
    std::string fname;
    fname = "../../data/tx77.hexstream";
    // auto words = siglabs::file::file_read_hex_stream(fname);


    // cout << "got words " << words.size() << "\n";

    // uint32_t* data = (uint32_t*)words.data();

    // for(int i = 0; i < words.size()/4; i++ ) {
    //     cout << HEX32_STRING(data[i]) << "\n";
    //     // cout << data[i] << "\n";
    // }


    fname = "../../data/tx77_data.raw";
    auto words2 = siglabs::file::file_read_raw_uint32_t(fname);
    cout << "got words " << words2.size() << "\n";

    // file_dump_vec(words2, "tx77_data.hex");
