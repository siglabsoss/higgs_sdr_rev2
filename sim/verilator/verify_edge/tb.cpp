...
int main(int argc, char** argv, char** env) {
  ...
  for(unsigned int i = 0; i < us; i++) {
    ...
    if(i == 20) {
        t->inStreamAppend("ringbusin", ringbus_udp_packet(6, 0x12000001 ) ); // output
    }

    if( i == 23) {

        t->inStreamAppend("ringbusin", ringbus_udp_packet(7, 0x11000001 ) ); // input
    }

    // unsigned int eth_pc = top->tb_higgs_top->eth_top->q_engine_inst->get_iBus_cmd_payload_pc();
    // if(
    //   eth_pc == pc_done[0] ||
    //   eth_pc == pc_done[1] ||
    //   eth_pc == pc_done[2]
