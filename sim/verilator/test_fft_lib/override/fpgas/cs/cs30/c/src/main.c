...
int main(void)
{
  ...
  report_test_results(0x1);

  test = test1();
  all_results |= (test==0)<<1;
  report_test_results(0x2);

  test = test2();
  all_results |= (test==0)<<2;
  report_test_results(0x3);

  test = test3();
  all_results |= (test==0)<<3;
  report_test_results(0x4);
  ...
  // delay to flush ringbus,  this test exits early once
  // we laeve main.  without this test appears to fail
