...
unsigned int enable_stream = FALSE;
unsigned int counter = 0;
unsigned int frame_num_all_output_period = 25000*4; //packet_num_SFO_adjustment_period
...
#define VEC_DEMOD_DATA_LENGTH (16)
// #define REAL_DATA //Comment for fake data
...
void setup_feedback_bus() {
    ...
    set_feedback_vector_length(&vec_demod_data, VEC_DEMOD_DATA_LENGTH/16); // 64
}
...
VMEM_SECTION unsigned int example_data[16] = {0xdeadbeef, 0x5a815a81, 0x5a815a81, 0x5a815a81, 0x5a815a81, 0x5a815a81, 0x5a815a81, 0x5a815a81, 0x5a815a81, 0x5a815a81, 0x5a815a81, 0x5a815a81, 0x5a815a81, 0x5a815a81, 0x5a815a81, 0x5a815a81};
...
void pet_mover() {
  ...
  if(incomming_occupancy > 0 && outgoing_dma_occupancy == 0) {
    ...
    /* Daniel - DST_ROW_REV*NSLICES calculates DMA address for data that has been demoved
    */
    ...
    dma_block_send(VMEM_DMA_ADDRESS(&vec_fine_sync), FEEDBACK_HEADER_WORDS);
    // dma_out_set(  VMEM_DMA_ADDRESS(&(input_dma[doff]))  , DMA_IN_EXTRA_USED);
    dma_block_send(  VMEM_DMA_ADDRESS(input_dma) + doff+FFT_SIZE, DMA_IN_EXTRA_USED);

//     vec_demod_data.seq = frame_track_counter;
//     dma_block_send(VMEM_DMA_ADDRESS(&vec_demod_data), FEEDBACK_HEADER_WORDS);
//     // vector_memory[VMEM_DMA_ADDRESS(empty_row)] = counter;
//     // counter++;

// #ifdef REAL_DATA
//     CSR_WRITE(DMA_1_START_ADDR, VMEM_DMA_ADDRESS(DST_ROW_REV*NSLICES));
// #else        
//     CSR_WRITE(DMA_1_START_ADDR, VMEM_DMA_ADDRESS(example_data));
// #endif
//     CSR_WRITE(DMA_1_LENGTH, VEC_DEMOD_DATA_LENGTH);
//     CSR_WRITE(DMA_1_TIMER_VAL, 0xffffffff);  // start right away
//     CSR_WRITE(SLICER,0x1);
//     CSR_WRITE_ZERO(DMA_1_PUSH_SCHEDULE);


    // dma_block_send(VMEM_DMA_ADDRESS(example_data),16);
