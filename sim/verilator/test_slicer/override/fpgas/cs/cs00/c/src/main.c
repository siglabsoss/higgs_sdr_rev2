...
void xbb_coarse_sync(CFO_Results *CFO_results, unsigned int coarse_sync_number) 
{
  ...
  for(int index=0; index<coarse_sync_number; index++)
  {
   ...
   // change on june 14
    ...
    ring_block_send_eth(COARSE_DEBUG_PCCMD | (temp_complex&DATA_MASK) );
  ...
  int sample_cp_advance_row_adjustment = 12;
  ...
  dma_in_set(dma_in_address,  1280);
...
void ring_block_send_eth_debug(unsigned int data)
{
  unsigned int occupancy;
  ...
  CSR_WRITE(RINGBUS_WRITE_DATA, RX_OVERFLOW | data);
...
void check_error_counter(void) {
    ...
    if(now-then > (125000000)) {
        // ring_block_send_eth_debug(riscv_status);
        // ring_block_send_eth(riscv_status);
        ring_block_send_eth(RX_UNDERFLOW|(riscv_status & 0xffffff));
        CSR_WRITE(CS_CONTROL, 0x1);
        CSR_WRITE(CS_CONTROL, 0x0);
