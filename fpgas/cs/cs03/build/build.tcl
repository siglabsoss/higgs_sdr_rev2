...
set impl_name "impl1"
set top_name "cs03_top";                                     # NOTE: bit file location & name will be ./impl_dir/top_name_impl_name.bit
set lpf_file "cs03_top.lpf";                                 # Lattice Logical Preferences

set syn_constraints "no";                                   # yes = pass Synplify specific constraints file to Synplify, no = no Synplify specific constraints file
set syn_constraints_file "cs03_top.fdc";                     # Synplify synthesis constraints (optional)

set insert_debug "no";                                     # yes = insert reveal debug core, no = skip debug core insertion
set debug_file "cs03_top.rvl";                               # project file for Reveal Analyzer cores to insert
...
prj_strgy set_value par_pathbased_place=On;
prj_strgy set_value partrce_worstcase_paths=4096;
prj_strgy set_value {syn_pipelining_retiming=Pipelining and Retiming}

if {[info exists env(BUILD_SEED)]} {
    prj_strgy set_value par_place_iterator_start_pt=$env(BUILD_SEED);
    puts "BUILD SEED: $env(BUILD_SEED)"
}
if {[info exists env(BUILD_ITERATIONS)]} {
    prj_strgy set_value par_place_iterator=$env(BUILD_ITERATIONS);
    prj_strgy set_value par_stop_zero=True;
    puts "BUILD ITER: $env(BUILD_ITERATIONS)"
}


#prj_strgy set_value {syn_vlog_input=System Verilog}
#prj_strgy set_value par_route_delay_reduction_pass=2;
#prj_strgy set_value par_routing_res_opt=2;
#prj_strgy set_value par_cdp=1;
#prj_strgy set_value par_routing_method=NBR;
#prj_strgy set_value par_place_iterator_start_pt=9;
#prj_strgy set_value par_place_iterator=5 ;
#prj_strgy set_value par_stop_zero=True;
