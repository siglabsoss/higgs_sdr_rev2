...
namespace signals {
  ...
  uint32_t constellation(uint32_t val);
	
      
  
      
	
  uint32_t two_over_sigma_sq(uint32_t val);
  ...
  Napi::Number constellationWrapped(const Napi::CallbackInfo& info);
      
  
      
  Napi::Number two_over_sigma_sqWrapped(const Napi::CallbackInfo& info);
